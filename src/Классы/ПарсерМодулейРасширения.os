...
Функция ПрочитатьМетодыМодуля(ФайлМодуля, Префикс = "") Экспорт
	...
	ДлинаИзменениеИКонтроль = 18;

	МетодыБезПрефикса = Новый Соответствие;
	...
	Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
		...
		СохраненноеТело = МетодыБезПрефикса[ИмяМетодаБезПрефикса];	
		Если СохраненноеТело <> Неопределено Тогда
			ТелоМетода = СохраненноеТело + Символы.ПС + Символы.ПС + ТелоМетода;
		КонецЕсли;	
		МетодыБезПрефикса.Вставить(ИмяМетодаБезПрефикса, ТелоМетода);
		Результат.Вставить(ИмяМетодаБезПрефикса,
			СтандартнаяСтруктураРезультата(ТипМетода, ИмяМетода, ИмяМетодаБезПрефикса, ТелоМетода));
...
//
Функция ПолучитьПереопределенныеОбъекты(ЭтоФорматEDT) Экспорт

	Результат = Новый Массив();

	Для Каждого ТипОбъектов Из ТипыОбъектов.ОбъектыКонфигурации Цикл

		ФайлыОбъектов = ПолучитьФайлыОбъектов(ТипОбъектов, ЭтоФорматEDT);
...
//   МодульРасширения - Структура - Свойства файла модуля, включая его методы, сюда добавляется сопостовление методов
...
Процедура ПрочитатьМетодыОсновнойКонфигурации(
	...
	Если МодульРасширения.Свойство("ИмяПереопределяемого", ИмяФайлаМодуляКонфигурации) Тогда
	...
	МодульРасширения.Вставить(ИмяКонфигурации, ФайлМодуляКонфигурации.Существует());
	
	Если МодульРасширения[ИмяКонфигурации] Тогда

		МетодыФайлаМодуля = ПрочитатьМетодыМодуля(ФайлМодуляКонфигурации);
		СопоставитьМетодыРасширенияИКонфигурации(МетодыФайлаМодуля, МодульРасширения, ИмяКонфигурации);
...
//
Функция ПолучитьФайлыОбъектов(ТипОбъектов, ЭтоФорматEDT) Экспорт

	КаталогОбъектов = ОбъединитьПути(РасширениеПуть, ТипОбъектов);
	Если ЭтоФорматEDT Тогда
		Файлы = НайтиФайлы(КаталогОбъектов, "*.mdo", Истина);
	Иначе
...
Процедура СопоставитьМетодыРасширенияИКонфигурации(МетодыМодуляКонфигурации, МодульРасширения, ИмяКонфигурации)

	МетодыМодуляРасширения  = МодульРасширения.Методы;
	Для каждого МетодКонфигурации Из МетодыМодуляКонфигурации Цикл // обход найденных методов

		МетодРасширения = МетодыМодуляРасширения[МетодКонфигурации.Ключ];

		Если МетодРасширения <> Неопределено Тогда
			МетодРасширения.Вставить(ИмяКонфигурации, МетодКонфигурации.Значение.ТелоМетода); 
		КонецЕсли;
...
Процедура ДобавитьПереопределенныйОбъект(ТипОбъектов, ФайлОбъекта, РезультатОбъекты)
	...
	Если Переопределение.Наследование Тогда
		...
		Если ТипОбъектов = "Configuration" Тогда
			ТипОбъектов = "";
		КонецЕсли;
		...
		Если ФайлыМодулей.Количество() Тогда
			...
			Для Каждого ФайлМодуля Из ФайлыМодулей Цикл
				...
				Если Не ПустаяСтрока(Переопределение.Значение) И СтрНайти(ФайлМодуля.Имя, ".bsl") > 0 Тогда
					ФайлСтруктура.Вставить("ИмяПереопределяемого", Переопределение.Значение);
				КонецЕсли;

				// в формах содержится информация о том какие методы были переопределены
				ФайлФормыМодуля = ФайлФормыМодуля(ФайлМодуля);
				Если ФайлФормыМодуля <> Неопределено Тогда
					ФайлСтруктура.Вставить("ФайлФормы", ФайлФормыМодуля);
				КонецЕсли;
...
///////////////////////////////////////////////////////////////////
Функция ФайлФормыМодуля(ФайлМодуля)
	Файлы = НайтиФайлы(ФайлМодуля.Путь, "Form.form", Ложь);
