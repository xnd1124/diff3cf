...
//
Процедура СохранитьРезультат(РезультатСравнения, ИмяФайлаРезультата, Лог, ФорматРезультата = "HTML", ПоказыватьТолькоВажныеИзменения = Ложь, kdiff3Path = "") Экспорт

	Если ФорматРезультата = "HTML" Тогда

		Попытка
			ГенераторОтчетаHTML.СохранитьРезультат(РезультатСравнения, ИмяФайлаРезультата, Лог, ПоказыватьТолькоВажныеИзменения);
		Исключение
	...
	ИначеЕсли ФорматРезультата = "TXT" Тогда
		...
		КонецЕсли;
	ИначеЕсли 	ФорматРезультата = "Kdiff" Тогда
		Попытка
			ГенераторОтчетаKdiff.СохранитьРезультат(РезультатСравнения, ИмяФайлаРезультата, Лог, ПоказыватьТолькоВажныеИзменения, kdiff3Path);
		Исключение
			ОбработкаИсключенияЗаписи(ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()), ИмяФайлаРезультата, Лог);
		КонецПопытки;
	Иначе 
